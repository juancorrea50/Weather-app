...
function convertDisplay(weatherInf,nodeCont){
  const conBtn = nodeCont.childNodes[0].childNodes[2];
  if(conBtn.textContent == '\u00B0C/KPH'){
    weatherInf.then(function(response){
      nodeCont.childNodes[0].childNodes[0].textContent = `Location: ${response.location.name}, ${response.location.country}`;
      nodeCont.childNodes[0].childNodes[1].textContent = `Temp: ${response.current.temp_c}\u00B0C`;
      ...
      nodeCont.childNodes[0].childNodes[2].textContent = '\u00B0F/MPH'
    })
  } else if(conBtn.textContent == '\u00B0F/MPH'){
    weatherInf.then(function(response){
      nodeCont.childNodes[0].childNodes[0].textContent = `Location: ${response.location.name}, ${response.location.country}`;
      nodeCont.childNodes[0].childNodes[1].textContent = `Temp: ${response.current.temp_f}\u00B0F`;
      ...
      nodeCont.childNodes[0].childNodes[2].textContent = '\u00B0C/KPH'
    })
  }
...
function weatherApp() {
  ...
  const convBtn = weather.childNodes[0].childNodes[2];
